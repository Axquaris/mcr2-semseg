...
def main():
    ...
    if 'mnist' in args.data:
        train_dataset = DigitSS(train=True, cifar_bg='bg' in args.data)
        val_dataset = DigitSS(train=False, cifar_bg='bg' in args.data)
        if '_bg' in args.data:
            im_channels = 3
        else:
            im_channels = 1
        early_stopper = pl.callbacks.EarlyStopping(monitor='val_instance_acc', min_delta=.008, patience=50)
    elif 'bdd' in args.data:
        train_dataset = BddSS(train=True)
        val_dataset = BddSS(train=False)
        im_channels = 3
        early_stopper = pl.callbacks.EarlyStopping(monitor='val_instance_acc', min_delta=.008, patience=2)
    else:
